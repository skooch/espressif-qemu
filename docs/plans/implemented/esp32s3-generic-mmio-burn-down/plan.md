...
- Modify: `hw/misc/esp32s3_ledc.c`
- Modify: `include/hw/misc/esp32s3_ledc.h`
...
## Tasks
- [x] Re-run the generic-MMIO trace classification when a new guest path or firmware revision touches the fallback.
- [x] Split source-backed deferred windows, such as LEDC or any new in-scope owner, out of the generic fallback into narrow explicit models.
- [x] Keep blocked radio/internal windows explicit in the docs instead of letting them drift into accidental compatibility behavior.
- [x] Ensure new catch-all addresses in core boot, reset, sleep, cache, clock, interrupt, eFuse, PMS, RNG, or Xtensa paths are treated as fidelity bugs.
- [x] Update `ESP32S3_EMULATION_GAPS.md` after each burn-down slice so the fallback boundary stays reviewable.
