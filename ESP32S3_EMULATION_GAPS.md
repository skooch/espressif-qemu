...
The immediate peripheral, deferred-foundation, core-SoC, first T-Deck prioritization plan, the 2026-04-18 follow-on clock/reset/sleep cleanup plan, the 2026-04-18 reset-fanout phase, the 2026-04-18 RTC time-trigger phase, the 2026-04-18 RTC light-sleep configuration phase, the 2026-04-18 RTC SDIO config phase, and plan `#1` (`docs/plans/implemented/esp32s3-board-control-realism/plan.md`) are now historical records under `docs/plans/implemented/`. Plan `#4`, `docs/plans/in-progress/esp32s3-external-memory-fidelity/plan.md`, is now the active ESP32-S3 fidelity queue. The remaining backlog is split into explicit `docs/plans/new/` plan records.
...
4. `docs/plans/in-progress/esp32s3-external-memory-fidelity/plan.md` for active cache/MMU, flash, and PSRAM coherency follow-ons
...
- SPI1 page program and erase commands now update the backing flash image but leave already visible EXTMEM flash aliases stale until a modeled maintenance path refreshes them. The first promoted coherency rule is `ICACHE_SYNC`: once the invalidate command completes, stale flash-backed alias pages become visible again in the EXTMEM mirror. This contract is now pinned by a direct MMIO qtest (`/xtensa/esp32s3/cache/flash-spi-requires-sync-for-mapped-alias`) and by the board-path flash regression (`tests/functional/test_xtensa_esp32s3_multicore_park_flash.py`), whose embedded ROM now parks the APP core, mutates flash, runs `ICACHE_SYNC`, and only then re-reads the mapped alias.
...
This is now partially SDK-contract accurate for the promoted coherency path: MMU programming, flash/PSRAM mapping, per-core bus gating, invalid-entry fault, reject delivery, cache-maintenance completion bits, and the first stale-before-sync / fresh-after-sync flash alias transition needed by the current firmware evidence. It is still not cycle-accurate cache hardware, and the wider coherency matrix is incomplete. DCache-backed coherency, PSRAM stale-line visibility, preload/autoload/freeze side effects, line fill/eviction/replacement policy, dirty-line writeback ordering, per-line suspend ownership, bus contention, pipeline stall timing, flash-controller micro-timing, encryption throughput timing, and the full cache PMS/access-mask matrix remain blocked for accuracy or still pending implementation.
