...
- Clock, reset, and sleep/power behavior are still only partially modeled. The `SYSTEM` block now carries the documented XTAL handoff plus the current firmware-visible wake clock contract across RTC light sleep, RTC sleep/wake logic is still tailored to current timer/GPIO/EXT1 paths, and reset now uses local guest software reset dispatch but still lacks full silicon reset-tree, analog, and retention sequencing.
