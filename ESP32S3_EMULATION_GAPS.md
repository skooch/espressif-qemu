...
- The active light-sleep path now freezes SYSTIMER in hardware, forces the documented XTAL clock source during RTC light sleep, restores the pre-sleep system clock on wake, and leaves elapsed-time compensation to the adjacent firmware wake path.
...
This is a QEMU-visible SoC transition contract and a guest-level recovery regression, not hardware proof of full ESP32-S3 low-power behavior. The local sleep reference still identifies broader missing behavior beyond the now-modeled XTAL handoff: stop XTAL/PLL and usually RC_FAST with real settling timing, compensate stopped SYSTIMER time on wake, model power-domain and peripheral gating decisions, and handle CPU retention/power-down variants. Those remain blocked for accuracy until each item is backed by the local TRM/ESP-IDF/esp-hal evidence or a hardware probe.
...
- RTC light sleep now forces `SYSTEM_SYSCLK_CONF.SOC_CLK_SEL` to XTAL while the RTC sleep state machine holds the SoC in light sleep, then restores the pre-sleep `SYSTEM_SYSCLK_CONF` and `SYSTEM_CPU_PER_CONF` state on wake. This matches the local sleep-reference requirement that the CPU switch away from PLL before sleep and resume its prior clock configuration afterward, without claiming PLL-startup or wake-stub timing accuracy.
...
| Clock / reset / sleep | Explicit narrow SDK contract for CPU/APB/RTC-selected clock propagation, TIMG APB/XTAL rate fanout, modeled RTC/reset flows, board-visible light-sleep CPU hold transitions, and XTAL handoff across RTC light sleep | PLL dynamics, oscillator stop/start timing, wake-stub timing, SYSTIMER sleep compensation policy, power-domain/peripheral gating, deep power behavior, and wider peripheral clock fanout remain incomplete | High |
...
- Clock, reset, and sleep/power behavior are still only partially modeled. The `SYSTEM` block now carries the documented XTAL handoff across RTC light sleep, RTC sleep/wake logic is still tailored to current timer/GPIO/EXT1 paths, and reset now uses local guest software reset dispatch but still lacks full silicon reset-tree, analog, and retention sequencing.
