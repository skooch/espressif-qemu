...
The immediate peripheral, deferred-foundation, core-SoC, first T-Deck prioritization plan, the 2026-04-18 follow-on clock/reset/sleep cleanup plan, the 2026-04-18 reset-fanout phase, the 2026-04-18 RTC time-trigger phase, the 2026-04-18 RTC light-sleep configuration phase, the 2026-04-18 RTC SDIO config phase, plan `#1` (`docs/plans/implemented/esp32s3-board-control-realism/plan.md`), plan `#2` (`docs/plans/implemented/esp32s3-peripheral-contract-follow-ons/plan.md`), plan `#3` (`docs/plans/implemented/esp32s3-generic-mmio-burn-down/plan.md`), plan `#4` (`docs/plans/implemented/esp32s3-external-memory-fidelity/plan.md`), plan `#5` (`docs/plans/implemented/esp32s3-emac-fidelity/plan.md`), plan `#6` (`docs/plans/implemented/esp32s3-rmt-model/plan.md`), and plan `#7` (`docs/plans/implemented/esp32s3-xtensa-backend-follow-ons/plan.md`) are now historical records under `docs/plans/implemented/`. There is currently no active in-progress ESP32-S3 fidelity plan; the remaining backlog is the documented source-gated residual set below rather than another queued numbered plan.
...
5. `docs/plans/implemented/esp32s3-emac-fidelity/plan.md` is the completed historical record for the source-backed `open_eth` wrapper contract and MII write sequencing
6. `docs/plans/implemented/esp32s3-rmt-model/plan.md` is the completed historical record for the explicit source-backed RMT register surface and first TX completion interrupt semantics
7. `docs/plans/implemented/esp32s3-xtensa-backend-follow-ons/plan.md` is the completed historical record for the residual Xtensa backend evidence gate and re-entry conditions
...
- The active light-sleep path now freezes SYSTIMER in hardware, forces the documented XTAL clock source during RTC light sleep, restores PLL selection on wake, returns `SYSTEM_CPU_PER_CONF` to the firmware-visible wake-stub boot clock contract (160 MHz PLL), and leaves elapsed-time compensation plus runtime tier reapplication to the adjacent firmware wake path.
...
| --- | --- | --- |
| Generic MMIO burn-down | Register-accurate for replaced SPI0/SPI_MEM, ASSIST_DEBUG, APB_SARADC, SENS, and the active ESP32-S3 LEDC register surface; compatibility shim for the remaining out-of-scope catch-all windows | Remaining FE/FE2/NRX/BB radio/internal catch-all hits are still deferred to their owning peripheral/radio phases; APB_SARADC/SENS/LEDC are register-surface shims only, not analog or timing behavior models |
...
The qtests `/xtensa/esp32s3/ledc/register-surface` and `/xtensa/esp32s3/generic-mmio/compatibility-storage` pin the current owner split. Stored readback is now restricted to the explicitly classified out-of-scope FE/FE2/NRX/BB radio/internal windows, with adjacent words remaining independent. The active ESP32-S3 LEDC offsets at `0x00`, `0x04`, `0x08`, `0x0c`, `0xa0`, `0xd0`, and `0xfc` are owned by a narrow explicit LEDC device model with source-backed reset defaults and write masks. Other unmapped fallthrough addresses such as `DR_REG_WCL_BASE + 0xf0` and unsupported LEDC offsets such as `DR_REG_LEDC_BASE + 0xf0` are deterministic read-as-zero/write-ignore while still emitting the trace events above. This is not a hardware-fidelity claim. It is a regression guard so future replacement of catch-all offsets with explicit models is deliberate and reviewable.
...
| `DR_REG_LEDC_BASE` | `0x00`, `0x04`, `0x08`, `0x0c`, `0xa0`, `0xd0`, `0xfc` | ESP-IDF `ledc_reg.h` present | Resolved by explicit narrow ESP32-S3 LEDC register-surface shim; PWM timing, counter progression, interrupts, and channel-output behavior remain unmodeled |
...
- Supported RTC_CNTL offsets: `0x000`, `0x004`, `0x008`, `0x00c`, `0x010`, `0x014`, `0x018`, `0x01c`, `0x020`, `0x038`, `0x03c`, `0x044`, `0x04c`, `0x050`-`0x05c`, `0x064`, `0x068`, `0x074`, `0x07c`, `0x080`, `0x084`, `0x088`, `0x08c`, `0x090`, `0x0b0`, `0x0b4`, `0x0b8`, `0x0bc`, `0x0c0`-`0x0cc`, `0x0d8`, `0x0dc`, `0x0e0`, `0x0e4`, `0x128`, `0x130`, `0x140`, `0x144`, `0x148`, `0x14c`, `0x150`, `0x154`, and `0x1fc`.
- Source-backed write masks are enforced for `OPTIONS0`, `TIMER2`, `SLP_TIMER1`, `WAKEUP_STATE`, `EXT_WAKEUP_CONF`, `SLP_REJECT_CONF`, `SDIO_CONF`, `RETENTION_CTRL`, `PG_CTRL`, `FIB_SEL`, `TOUCH_DAC`, `TOUCH_DAC1`, `COCPU_DISABLE`, `RTC`, `PWC`, `BIAS_CONF`, `REGULATOR_DRV_CTRL`, `DIG_PWC`, `SWD_CONF`, `PAD_HOLD`, `EXT_WAKEUP1`, and `DATE`. Write-only bits such as `MAIN_TIMER_ALARM_EN`, `SWD_FEED`, and `EXT_WAKEUP1_STATUS_CLR` no longer appear as generic readback.
- `RTC_CNTL_TIMER1` at `0x01c` now has the documented `PLL_BUF_WAIT`, `XTL_BUF_WAIT`, and `CK8M_WAIT` fields with TRM/ESP-IDF reset defaults and write masks. QEMU still does not turn those fields into analog wake timing or oscillator settling delays; this is an explicit register-surface contract only.
- `RTC_CNTL_EXT_WAKEUP1_STATUS` at `0x0e4` now latches the RTC GPIO bitmap that satisfied the current EXT1 wake condition, both for immediate EXT1 wake and for transitions while already sleeping. Writing `RTC_CNTL_EXT_WAKEUP1_STATUS_CLR` through `RTC_CNTL_EXT_WAKEUP1` clears that latched bitmap without turning the register into generic compatibility storage.
- `RTC_CNTL_SLP_REJECT_CAUSE` at `0x128` now latches the currently modeled reject source for the active board path. Today that means the GPIO low-level pre-sleep reject path records `RTC_GPIO_TRIG_EN`, and `SLP_REJECT_CAUSE_CLR` clears the latched value when the reject state is cleared. The wider reject-trigger matrix remains unmodeled until those sources are implemented.
- `RTC_CNTL_RETENTION_CTRL` at `0x140` now has the documented `RETENTION_WAIT`, `RETENTION_EN`, `RETENTION_CLKOFF_WAIT`, `RETENTION_DONE_WAIT`, `RETENTION_CLK_SEL`, `RETENTION_TARGET`, and `RETENTION_TAG_MODE` fields with TRM reset defaults and write masks. QEMU still does not turn that register into retention DMA setup, retention-memory save/restore, or CPU power-down sequencing; this is an explicit register-surface closure only.
- `RTC_CNTL_PG_CTRL` at `0x144` now has the documented `POWER_GLITCH_EN`, `POWER_GLITCH_EFUSE_SEL`, `POWER_GLITCH_FORCE_PU`, `POWER_GLITCH_FORCE_PD`, and `POWER_GLITCH_DSENSE` fields with write masks and reset default zero. QEMU does not model analog FIB behavior, power-glitch injection, or brownout side effects from those bits; this is an explicit register-surface closure only.
- `RTC_CNTL_FIB_SEL` at `0x148` now has the documented three-bit reset default and write mask. QEMU does not manufacture glitch, brownout, or super-WDT reset sources from that field; it remains an explicit register-surface closure only.
- `RTC_CNTL_TOUCH_DAC` / `RTC_CNTL_TOUCH_DAC1` at `0x14c` / `0x150` now preserve the documented touch-pad DAC fields with write masks and reset default zero. QEMU does not model touch-sensor analog behavior from those values; this is explicit register storage only.
- `RTC_CNTL_COCPU_DISABLE` at `0x154` now preserves the documented `DISABLE_RTC_CPU` bit with reset default zero. QEMU still does not instantiate or schedule the RTC coprocessor, so this is only the register contract.
...
- Source-known but unmodeled RTC_CNTL offsets are deterministic unsupported behavior: reads return zero and writes are ignored. This includes `0x024`-`0x034`, `0x040`, `0x048`, `0x060`, `0x06c`-`0x070`, `0x078`, `0x094`-`0x0ac`, `0x0d0`-`0x0d4`, `0x0e8`-`0x124`, `0x12c`, `0x134`-`0x13c`, and any undefined gap up to `0x1f8`. The qtest suite now pins `INT_ENA_W1TC` at `0x13c` as the unsupported-register sentinel for this RTC pass.

This is SDK-contract and board-path accurate for the modeled light-sleep wake/reject flows. It is not a full ESP32-S3 power manager. Full internal power-domain sequencing, CPU-retention DMA timing, retention-memory save/restore, brownout interactions, analog reset behavior, RTC watchdog escalation timing, ULP/touch/USB wake behavior, and oscillator/settling delays remain blocked for accuracy without more detailed source evidence or hardware probes.
...
- Direct qtests cover timer sleep asserting `light-sleep` until wake, GPIO sleep rejection never asserting `light-sleep`, RTC CPU-stall output behavior, SYSTEM core1 RUNSTALL output behavior, and EXT1 wake-status latching and clear behavior.
...
This is a QEMU-visible SoC transition contract and a guest-level recovery regression, not hardware proof of full ESP32-S3 low-power behavior. The local sleep reference still identifies broader missing behavior beyond the now-modeled XTAL handoff, wake-stub clock contract, and documented RTC wait-register surface: stop XTAL/PLL and usually RC_FAST with real settling timing, compensate stopped SYSTIMER time on wake, model power-domain and peripheral gating decisions, and handle CPU retention/power-down variants. Those remain blocked for accuracy until each item is backed by the local TRM/ESP-IDF/esp-hal evidence or a hardware probe.
...
- SPI1 page program and erase commands now update the backing flash image but leave already visible EXTMEM flash aliases stale until a modeled maintenance path refreshes them. The promoted coherency rules for flash-backed aliases are now: `ICACHE_SYNC` invalidate refreshes stale pages in-range, `ICACHE_PRELOAD` refreshes stale pages in its programmed range, `ICACHE_AUTOLOAD` refreshes stale pages in its programmed section ranges, and rewriting an MMU entry reloads the newly mapped flash page into the visible mirror. These contracts are pinned by direct qtests (`/xtensa/esp32s3/cache/flash-spi-requires-sync-for-mapped-alias`, `/xtensa/esp32s3/cache/flash-mmu-remap-reloads-visible-alias`, and `/xtensa/esp32s3/cache/preload-autoload-refresh-stale-aliases`) plus the board-path flash regression (`tests/functional/test_xtensa_esp32s3_multicore_park_flash.py`), whose embedded ROM now parks the APP core, mutates flash, runs `ICACHE_SYNC`, and only then re-reads the mapped alias.
- PSRAM-backed mappings are read/write IOMMU mappings into a guest-visible PSRAM mirror when the `esp32s3` machine has nonzero `-m` memory. Without PSRAM, a PSRAM-typed MMU entry gives no translation permission. Raw SPI1 PSRAM writes now mutate the backing RAM and can leave mapped DBUS aliases stale until `DCACHE_SYNC` invalidate reloads the page from backing PSRAM; DBUS alias writes now stay local to the visible mirror until `DCACHE_SYNC` writeback or clean pushes them into the backing RAM. These contracts are pinned by `/xtensa/esp32s3/cache/psram-raw-write-requires-dcache-invalidate` and `/xtensa/esp32s3/cache/psram-alias-write-requires-dcache-writeback`.
...
This is now functionally SDK-contract accurate for the promoted coherency path: MMU programming, flash/PSRAM mapping, per-core bus gating, invalid-entry fault, reject delivery, cache-maintenance completion bits, flash stale-before-maintenance / fresh-after-`ICACHE_SYNC`-`PRELOAD`-`AUTOLOAD` visibility, PSRAM stale-before-`DCACHE_SYNC` invalidate visibility, PSRAM alias-write-before-`DCACHE_SYNC` writeback visibility, and MMU-remap refresh behavior. It is still not cycle-accurate cache hardware. DCache preload/autoload range semantics remain outside the current claim because the local ESP32-S3 register model does not expose source-backed address/size registers for those controls, and line fill/eviction/replacement policy, dirty-line ownership, bus contention, pipeline stall timing, flash-controller micro-timing, encryption throughput timing, and the full cache PMS/access-mask matrix remain blocked for accuracy or intentionally out of scope.
...
- RTC light sleep now forces `SYSTEM_SYSCLK_CONF.SOC_CLK_SEL` to XTAL while the RTC sleep state machine holds the SoC in light sleep, then returns the clock block to the adjacent firmware-visible wake contract on exit: `SYSTEM_SYSCLK_CONF` selects PLL again and `SYSTEM_CPU_PER_CONF` returns to the wake-stub boot configuration (160 MHz PLL) before firmware reapplies its desired runtime tier. This matches the local sleep-reference and adjacent firmware ownership boundary without claiming PLL-startup or wake-stub timing accuracy.
...
This phase does not model analog PLL lock, oscillator startup, XTAL/PLL stop-start timing, jitter, DFS/source-switch latency, divider settling, wake-stub restore timing, peripheral clock-domain crossing outside TIMG, SYSTIMER compensation policy, or undocumented divider interactions. Those are blocked for accuracy without stronger timing references or hardware probes.
...
| Generic MMIO surface | Instrumented and narrower than before, with only FE/FE2/NRX/BB fallback storage left | Unknown deferred radio/internal registers can still appear to work via stored readback unless traced and replaced with explicit models | Medium |
| Clock / reset / sleep | Explicit narrow SDK contract for CPU/APB/RTC-selected clock propagation, TIMG APB/XTAL rate fanout, modeled RTC/reset flows, board-visible light-sleep CPU hold transitions, and the current firmware-visible XTAL-to-PLL wake clock contract across RTC light sleep | PLL dynamics, oscillator stop/start timing, wake-stub timing, SYSTIMER sleep compensation policy, power-domain/peripheral gating, deep power behavior, and wider peripheral clock fanout remain incomplete | High |
...
| USB Serial/JTAG, I2C, UART, SHA | Board-path complete with RX, completion semantics, clock-derived timing, explicit unsupported I2C mode handling, and IRQ coverage | DMA error paths, uncommon timing modes, and multi-CPU interrupt routing remain incomplete | Medium |
...
| Ethernet | Generic `open_eth` stand-in with direct link/MII, staged PHY write sequencing, and descriptor loopback regression coverage | Still not an ESP32-S3-specific EMAC model or full PHY implementation | Low for the current workload, Medium to High if future firmware depends on deeper EMAC details |
| RMT | Explicit narrow register-surface model with immediate TX_END completion interrupts for TX start | Waveform timing, RX capture, GDMA ownership, and full channel behavior remain unmodeled | Low for the current workload, Medium to High if future firmware depends on deeper RMT behavior |
...
- Unimplemented MMIO is now much less likely to be papered over accidentally. The remaining catch-all register window stores writes and echoes reads only for the explicitly classified out-of-scope FE/FE2/NRX/BB radio/internal ranges; other unmapped fallthrough addresses are RAZ/WI while still traceable through `esp32s3_unimplemented_io_read` and `esp32s3_unimplemented_io_write`. Phase 1 classified the active catch-all hits and replaced the boot-critical SPI0/SPI_MEM and core-debug ASSIST_DEBUG hits with explicit narrow models, Phase 2 replaced the APB_CTRL RAM island with an explicit narrow model, P1 follow-on work replaced APB_SARADC/SENS active offsets with source-backed register-surface shims, plan `#3` moved the active ESP32-S3 LEDC offsets into a dedicated narrow LEDC device with source-backed reset defaults and write masks before limiting fallback storage to the deferred radio/internal ranges, and plan `#6` replaced the explicit RMT unimplemented stub with a narrow source-backed register surface plus immediate TX_END completion interrupts for TX start on channels 0 through 3. The ANA PLL-ready behavior remains a named compatibility shim that forces firmware-visible ready bits high so polling loops keep moving.
- GP-SPI now reads chip-select and DC signals through the routed-signal layer (`esp32s3_gpio_get_routed_signal_level`) rather than hard-coded GPIO numbers. The default T-Deck signal-to-pin assignments (EPD_CS→GPIO34, EPD_DC→GPIO35, SD_CS→GPIO48, LoRa_CS→GPIO3) are now explicit SoC/board-owned defaults that are reapplied across digital resets instead of being hidden in generic GPIO reset state.
- SPI2/SPI3 no longer complete transfers unconditionally in the same write. The current staged GP-SPI model now defers `USR` clear and `TRANS_DONE` through a small two-phase completion timer, but broader busy-window realism, DMA corner cases, and controller timing are still simplified.
- Clock, reset, and sleep/power behavior are still only partially modeled. The `SYSTEM` block now carries the documented XTAL handoff plus the current firmware-visible wake clock contract across RTC light sleep, RTC sleep/wake logic now includes the active EXT1 status latch path but is still tailored to current timer/GPIO/EXT1 paths, and reset now uses local guest software reset dispatch but still lacks full silicon reset-tree, analog, and retention sequencing.
- Most previously placeholder peripherals have been tightened to board-path-complete behavior: USB Serial/JTAG now supports RX delivery, FIFO-used status reporting, and RX/TX interrupt state; I2C clears and sets DONE bits per-command with deferred completion IRQ delivery and now treats unsupported slave-mode/APB-nonfifo transfer starts as an explicit no-op contract; UART derives baud and pulse timing from the active clock configuration (falling back to 40 MHz only when no clock device is linked); PMS now has an explicit source-backed register table for the modeled surface and RAZ/WI behavior elsewhere; SHA asserts and clears interrupt state on completion for both DMA and non-DMA paths; RNG is intentionally narrow with one 32-bit host-backed data register and deterministic ignored writes. Remaining gaps include DMA error paths, uncommon timing modes, and multi-CPU interrupt routing.
...
- Some blocks are substituted with generic IP rather than an S3-specific model. Ethernet still uses `open_eth`, but the current tree now makes the QEMU-visible contract explicit: `MIICOMMAND`/MII link polling stays latched correctly, `MIITX_DATA` stages PHY writes until `MIICOMMAND.WCTRLDATA`, backend link toggles update `MIISTATUS`, and loopback mode can drive descriptor RX from TX for direct regression coverage.
...
- The generic Xtensa backend still has known accuracy gaps such as the residual reject-surface matrix and the generic unimplemented-opcode fallback, but no new active guest blocker is currently reproduced beyond the already-covered `S32C1I`/`ATOMCTL` and board reject paths.
...
- The adjacent firmware's RTC light-sleep configuration registers now have explicit masks and reset defaults for the source-backed `OPTIONS0`, `TIMER2`, `CLK_CONF`, `SDIO_CONF`, `RTC`, `PWC`, `BIAS_CONF`, `REGULATOR_DRV_CTRL`, and `DIG_PWC` surface. Broader oscillator stop/start timing and power-domain sequencing are still incomplete.
- The active light-sleep path now freezes SYSTIMER, forces the documented XTAL system-clock source during RTC sleep, restores the firmware-visible PLL-plus-boot-clock wake state on exit, and still leaves elapsed-time compensation plus runtime tier reapplication to the adjacent firmware. Broader oscillator stop/start timing and peripheral-gating realism remain incomplete.
...
- The RTC-to-clock ownership split is now explicit: the RTC block owns only RTC-local `CLK_CONF` state, while CPU/system `SOC_CLK_SEL` remains owned by `SYSTEM_SYSCLK_CONF` and its SoC clock-update path.
...
- Multi-core flash writes and erases rely on explicit Core 1 parking around the ROM flash calls. The active firmware uses `FlashStorage::multicore_auto_park()` for littlefs and BLE OTA, so the guest-visible contract we care about first is "park the other core, perform the flash op, run the cache maintenance that makes the mutation visible, then unpark" rather than a cycle-accurate cache-disable implementation. The board-path functional regression `tests/functional/test_xtensa_esp32s3_multicore_park_flash.py` now exercises this full contract end-to-end: APP-core counter loop, RTC_CNTL stall-magic park, SPI1 WREN+PP and WREN+SE with frozen counter, explicit `ICACHE_SYNC`, EXTMEM ICACHE-alias coherence, and APP-core counter resume after unpark.
...
- Current tree: `open_eth` now latches `MIICOMMAND`, preserves `SCANSTAT`-driven link polling behavior across host reads, stages `MIITX_DATA` until `MIICOMMAND.WCTRLDATA` before mutating the emulated PHY, updates `MIISTATUS.LINKFAIL` when the backend link changes, and supports descriptor TX-to-RX loopback when `MODER.LOOPBCK` is enabled.
- Current coverage: the ESP32-S3 qtest suite now boots the board with `-nic user,id=emac0,model=open_eth`, uses QMP `set_link` to verify MII-visible link up/down transitions, proves that PHY writes do not commit until `WCTRLDATA` is asserted, and proves TX/RX descriptor plus IRQ behavior through loopback.
...
- Firmware that touches only the currently exercised board path is much more likely to work than firmware that depends on deep power-management, uncommon DMA/peripheral corner cases, EMAC behavior beyond the current OpenCores contract, full RMT waveform/capture behavior, or architectural edge conditions.
...
- Status:
  - Done: IO_MUX device, routed signal plumbing for EPD/SD/LoRa control pins, explicit T-Deck board-owned default routing for those signals across reset, richer staged GP-SPI transfer semantics, and qtest coverage for default + non-default remap state.
...
- Status:
  - Done: USB Serial/JTAG RX/FIFO-ready path, I2C DONE semantics plus explicit ignored slave/APB-nonfifo starts, clock-derived UART timing, narrow RNG/PMS behaviors, SHA completion IRQ handling, and GP-SPI completion staging.
