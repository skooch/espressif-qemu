...
- DBUS MMU rewrites that touch a PSRAM-typed entry while `DCACHE_FREEZE_ENA` is not set emit an `info_report` log line so the contract violation is visible; hardware does not expose a fault signal for this case, so enforcement is observability-only. This keeps QEMU from inventing a divergent register bit while still flagging guests that skip the ROM `Cache_Suspend_DCache()` prologue.
